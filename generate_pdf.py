...
def get_stock_details(symbol=SYMBOL):
    ...
    from test_info import calculate_beta, get_company_info
    beta_value = calculate_beta(symbol)
    
    # Get shares outstanding (placeholder - implement actual data fetching)
    shares_outstanding = get_company_info(symbol,"outstanding_share")  # Example value
...
def draw_share_details(c, details, y_position):
    ...
    ("Đơn vị tiền", "VND"),
    ("Cổ phiếu lưu hành", f"{details['Cổ phiếu lưu hành']:,.3f}")
]
...
def main():
    ...
    # Create PDF
    c = canvas.Canvas("MWG.pdf", pagesize=A4)
    # Draw content
